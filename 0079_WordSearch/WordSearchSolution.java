        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++){
                if (searchWord(board, word, 0, i, j))
                    return true;
                // 错误写法！
//                这样其实就变成只从坐标 (0,0) 开始搜索，搜索不到返回 False，但题目的意思是：只要你的搜索返回 True 才返回，如果全部的格子都
//                搜索完了以后，都返回 False ，才返回 False。
//
//                作者：liweiwei1419
//                链接：https://leetcode-cn.com/problems/word-search/solution/zai-er-wei-ping-mian-shang-shi-yong-hui-su-fa-pyth/
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < words.length; i++)
            if ((new WordSearchSolution()).exist(b1, words[i]))
        ...
        char[][] b2 = {{'A'}};
        if ((new WordSearchSolution()).exist(b2, "AB"))
